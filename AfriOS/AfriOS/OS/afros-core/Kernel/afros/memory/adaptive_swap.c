...
afros_status_t memory_swap_out(afros_virt_addr_t page_addr) {
    kprintf("[MEM-SWAP] Inactive page detected at %p. Compressing...\n", (void*)page_addr);
    ...
    if (status == AFROS_SUCCESS) {
        kprintf("[MEM-SWAP] Compression réussie. Gain: ~60%%.\n");
    } else {
        kprintf("[MEM-SWAP] Echec compression. Utilisation du swap disque classique.\n");
    }
...
afros_status_t memory_swap_in(afros_virt_addr_t compressed_addr) {
    kprintf("[MEM-SWAP] Fault on compressed page. Decompressing...\n");
