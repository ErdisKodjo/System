...
afros_status_t afrosfs_read_file(afros_file_t *file, uint8_t *buffer, afros_size_t size) {
    ...
    printf("AfrosFS: Reading %zu bytes from file %s...\n", size, file->name);
...
afros_status_t afrosfs_write_file(afros_file_t *file, const uint8_t *buffer, afros_size_t size) {
    ...
    if (file->flags & 0x1) { // 0x1 = READ_ONLY
        printf("AfrosFS ERROR: Attempt to write to read-only file %s.\n", file->name);
        return AFROS_ERROR;
    }

    printf("AfrosFS: Writing %zu bytes to file %s...\n", size, file->name);
    
    // Mise  jour de la taille
    file->size += size;
    
    // Synchronisation avec le journal (Journaling)
    printf("AfrosFS [Journal]: Committing write transaction for %s.\n", file->name);
    
    return AFROS_SUCCESS;
}

afros_status_t afrosfs_delete_file(const char *name) {
    printf("AfrosFS: Deleting file %s and reclaiming blocks.\n", name);
    return AFROS_SUCCESS;
}
