...
void power_check_solar_status(void) {
    ...
    if (source == AFROS_POWER_SOURCE_SOLAR) {
        printf("[POWER] Source solaire détectée. Activation du mode HAUTE PERFORMANCE.\n");
        printf("[POWER] Action : Fréquences CPU maximales autorisées.\n");
        
        // Exemple : Augmenter la fréquence du Core 4 (big)
        arch_cpu_ops.set_frequency(4, 2800);
    } else {
        printf("[POWER] Source Batterie/AC. Passage en mode ECONOMIE D'ENERGIE.\n");
        printf("[POWER] Action : Limitation des fréquences CPU.\n");
